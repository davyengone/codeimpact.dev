...
// Technology data with proper IDs
const technologies = [
...
async function migrateTechnologies() {
  ...
  console.log(`📊 Will insert ${technologies.length} technologies`)
  ...
  try {
    ...
    const { data: columns } = await supabase
      ...
      .select('column_name')
      .eq('table_name', 'technologies')
      .eq('column_name', 'slug')

    if (!columns || columns.length === 0) {
      // Add slug column - we'll use raw SQL via a function call
      console.log('Adding slug column...')
      // For now, let's proceed without the schema change and just clear/repopulate
    }

    // Step 2: Clear existing technologies
    ...
    // Step 3: Insert new technologies in batches
    ...
    for (let i = 0; i < technologies.length; i += batchSize) {
      ...
      console.log(`✅ Inserted batch ${Math.floor(i / batchSize) + 1}: ${batch.length} technologies (${batch[0].name} - ${batch[batch.length - 1].name})`)
    }

    console.log(`🎉 Migration completed successfully!`)
    console.log(`📊 Successfully inserted ${successCount}/${technologies.length} technologies`)
    ...
    if (!countError && finalCount !== null) {
      console.log(`✅ Verification: ${finalCount} technologies in database`)
    }

    // Show a sample of inserted technologies
    const { data: sample } = await supabase
      .from('technologies')
      .select('name, category')
      .limit(10)

    console.log('📋 Sample of inserted technologies:')
    sample?.forEach(tech => {
